...
void loop() {
  ...
  // Perform detection algorithm if am obstacle is detected
  if (detect_obstacle()) {
    detection_algorithm();
  }
}
...
void printTemperature() {
  ...
  delay(2000); // Delay 2s if values are read in a loop
}
...
boolean detect_obstacle() {
 if(distanceToClosestObject() <= 10.0) {
     return true;
 }
 return false;
}
